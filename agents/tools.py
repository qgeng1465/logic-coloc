...
同源度评分必须遵守量表：90–100% 表示底层机制几乎完全对应；70–89% 表示共享宏观结构但实现有差异；50–69% 表示抽象概念相似但机制有本质区别；0–49% 表示强行类比、逻辑不成立。报告必须在 mechanism_summary、intersection_lesson 或 verdict_reason 中明确说明评分依据；不得自行编造 retrieval_score 或 homonomy_score。
必须积极寻找跨学科的底层逻辑同源。神经网络（计算机）与生物神经网络（神经科学）、负反馈（控制论）与生态稳定（生态学）都是完全合理的类比；绝对不允许因为“不是科学等价”就草率拒绝跨学科映射。类比是为了辅助理解，不是追求数学等价。请在有可解释的机制交集时明确给出映射，并把差异和边界放进严谨性说明。
