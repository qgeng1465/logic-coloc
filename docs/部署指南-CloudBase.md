...
| `LC_MAX_GUESTS` | `2000`（默认值，可不配） | 匿名身份上限。见下方「关于匿名身份」 |
...
**打开网站就能用**：前端首次访问会静默调 `POST /api/auth/guest`，服务端发一个匿名
身份（一个 uid），笔记/卡片/能量照常可用，各人数据互不可见。登录页只在**拿不到匿名
身份**（断网、后端没起来）时作为兜底出现。用户在「设置 → 保存我的知识」里补一个用户名
密码，就能把当前 uid 升级成正式账号 —— **uid 不变，数据不丢**。

对部署有两处影响，都不需要额外配置，但要知道：

1. **`data/users.json` 会被陌生访客持续追加**（每个新浏览器一条匿名记录）。这是本站
   唯一一个匿名可写磁盘的入口，所以由 `LC_MAX_GUESTS`（默认 2000）封顶：超限时**只从
   `users.json` 里摘掉最早的那条记录**，磁盘上的 `data/users/<uid>/`、`uploads/<uid>/`
   不删（删文件不可逆）。被摘掉的那个 token 从此校验不过，等价于「很久没来的匿名身份
   过期了」。**正式账号永不参与淘汰。** 演示场景默认值足够，不用动。
2. **匿名身份也在 `data/` 里**，所以第 7 节第 1 条那个持久化卷同时是「游客数据别丢」的
   前提 —— 不挂卷的话，重新部署会把所有匿名用户连同正式账号一起清空。
...
**3. 匿名身份能建出来（本站免登录，这一步最容易在后面才发现坏）**

```bash
curl -s -X POST https://你的域名/api/auth/guest
```

期望 `code:0` + 一个 `token`，且 `user.guest` 是 `true`。拿这个 token 去访问一个受保护
接口，应该是 `200` 而不是 `401`：
...
浏览器打开域名 —— **不该出现登录页**，应该直接进应用。页面上也不该出现「模型服务未连接」。
如果弹了登录页，说明匿名身份没建出来（多半是后端没起来或断网），看第 6 节。
...
| **打开网站就弹登录页**（本该免登录直接用） | 匿名身份没建出来 —— 多半是断网或后端没起来，前端才退回登录页 | 先 `curl -X POST /api/auth/guest` 看有没有 `token`；没有就是后端的问题，**不是登录逻辑坏了** |
| 登录/注册成功但刷新后又回到登录页 | 浏览器禁用了 `localStorage`（隐私模式、部分内嵌 WebView） | token 存在 `localStorage` 里，禁用就没法保持登录；游客身份同样存不下，会每次访问都换一个新身份 |
| 匿名用户的数据在重新部署后没了 | 没挂持久化卷 | 匿名身份也住在 `data/users/<uid>/` 里，和正式账号一样，见第 7 节第 1 条 |
| 老游客的 token 突然失效 | 匿名记录数超过 `LC_MAX_GUESTS`，最早的被淘汰了 | 预期行为，直接刷新即可拿到新身份；正式账号不受影响 |
